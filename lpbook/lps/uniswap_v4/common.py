...
from lpbook.lps.uniswap_v4.subgraph import UniV4GraphQLClient
...
class UniV4LikeTheGraphProxy(LPAsyncProxy):
    """Loads the state of liquidity from TheGraph."""
    def __init__(self, lp_ids, uniswap_v4_gql_client):
        assert len(lp_ids) >= 1
        self.lp_ids = lp_ids
        self.client = uniswap_v4_gql_client

    def create_from_thegraph(self, thegraph_data, block: BlockId) -> Optional[UniV4Like]:
        try:
            id_ = thegraph_data.id
            token0 = Token(
            ...
            univ4 = UniV4Like(
                ...
                token0=token0,
                token1=token1,
                tick_spacing=int(thegraph_data.tick_spacing),
        ...
        return univ4 if len(univ4.liquidity_net) > 0 else None
    ...
    async def __call__(
        ...
        logger.debug(
            f'Retrieving uniswap V4 state from TheGraph at block {block} ...'
        )
        ...
        if state.keys() != set(self.lp_ids):
            logger.error(f"Univ4 sync solver did not return all lp_ids: missing {set(self.lp_ids) - set(state.keys())}")

        return state


class UniV4LikeTheGraphAndWeb3Proxy(LPFromInitialStatePlusChangesProxy):
    """Queries TheGraph for an initial state, and web3 for state updates."""

    def __init__(self, lp_ids, async_proxy, event_stream, web3_client, abi_filename, pool_manager):
        self.lp_ids = lp_ids

        # read abi from same directory as this file.
        with open(Path(__file__).parent / 'artifacts' / abi_filename, 'r') as f:
            contract_abi = f.read()
        UniV4 = web3_client.eth.contract(abi=contract_abi)

        super().__init__(
            [pool_manager],
            [UniV4.events.Swap, UniV4.events.ModifyLiquidity],
            async_proxy,
            event_stream,
            web3_client,
            extra_topics=([bytes.fromhex(lp_id[2:]) for lp_id in self.lp_ids],)
        )
    
    def update_state(self, state: Dict[str, LP], d: Any) -> None:
        lp_id = "0x" + d.args.id.hex()
        ...
        if d.event == 'Swap':
        ...
        elif d.event == 'ModifyLiquidity':
            tick_lower = d.args.tickLower
            tick_upper = d.args.tickUpper
            liquidity_delta = d.args.liquidityDelta
            ...
            lp.liquidity_net[tick_lower] += amount
            lp.liquidity_net[tick_upper] -= amount
    ...
    def get_trades(self, cur_state: Dict[str, LP], changes: List[Any]) -> list[Trade]:
        ...
        for d in changes:
            ...
            trade = Trade(
                ...
                buy_amount1=-d.args.amount0,    # UniV4 swapped the signal of amounts
...
class UniV4LikeDriver(LPDriver):
    def __init__(
        self,
        ...
        super().__init__(self.UniV4Like)
        self.event_stream = event_stream
        self.block_stream = block_stream
        self.web3_client = web3_client
        self.graphql_client = UniV4GraphQLClient(self.thegraph_url, session)
    ...
    async def create_lp_sync_proxy(
        ...
        async_proxy = self.UniV4LikeTheGraphProxy(lp_ids, self.graphql_client)
        if data_source in [
            LPDriver.LPSyncProxyDataSource.Default,
            LPDriver.LPSyncProxyDataSource.TheGraphAndWeb3
        ]:
            sync_proxy = self.UniV4LikeTheGraphAndWeb3Proxy(
                lp_ids,
                async_proxy,
                self.event_stream,
                self.web3_client,
    ...
    def create_lp_async_proxy(
        ...
        if data_source in [
            ...
            return self.UniV4LikeTheGraphProxy(
                lp_ids, self.graphql_client
            )

        raise RuntimeError("Invalid data_source for async_proxy to UniV4")
